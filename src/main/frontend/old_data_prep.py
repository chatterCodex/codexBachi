...
def prepare_map_data(forest_area_3, results_df: pd.DataFrame, model_list=None) -> Dict[str, Any]:
    ...
    for geom, bhd in zip(gtrees, bhd_series):
        ...
        tree_x.append(x)
        tree_y.append(y)
        tree_bhd_cm.append(None if pd.isna(bhd) else int(bhd))
    ...
    vol_col_candidates = ["wood_volume", "colume_m3", "volumen_m3", "volume"]

    if forest_area_3.harvesteable_trees_gdf.iloc[]
