...
from src.main.frontend.data_prep import make_radar_scores, get_overview_table_data, get_selected_table_data, get_anchor_table_data
...
def build_interface(forest_area_3, model_list, results_df: pd.DataFrame) -> w.VBox:
    ...
    selector = ResultSelector(num_results=len(results_df), on_select=lambda idx: _on_select(idx, forest_area_3, model_list, results_df, selected_table, anchor_table))
...
def _on_select(selected_index, forest_area_3, model_list, results_df, selected_table: Table, anchor_table: Table):
